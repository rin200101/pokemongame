...
function loadImage() {
    var list = new Array;
    var i;
    for (i = 1; i <= 36; i++) list.push(oStaticURL + "images/pieces" + i + ".jpg");
...
function newPiece(board, col, row, value) {
    ...
    div.setImage = function(imgIndex) {
        this.image.src = oStaticURL + "images/pieces" + imgIndex + ".jpg";
...
function newBoard() {
    ...
    for (var i = 0; i < 3; i++) {
        ...
        div.lines[i].style.backgroundColor = "#ECB365";
    ...
    div.createNewGame = function() {
        if (checksound) {
            getAudioName("sound4").play();
        }
        this.level = 1;
        this.blood = 30;
        this.score = 0;
        ...
        if (mybutton == 0) {
            if (this.isWaiting) {
                ...
                if (list != null) {
                    ...
                    this.drawPath(list);
                    setTimeout("afterDrawPath()", 500);
                    if (checksound) {
                        getAudioName("sound5").play();
                    }
                } else {
                    if (checksound) {
                        getAudioName("sound8").play();
                    }
    ...
    div.disp_confirm = function() {
        this.createNewGame();
    }
    div.pas_confirm = function() {
        alert("Pause game");
    }
    div.changesound = function() {
        var change_soundID = jh("#change-sound");
        if (checksound) {
            checksound = false;
            change_soundID.val("");
            change_soundID.attr("class", "soundOff");
            alert("Sound Off");
        } else {
            checksound = true;
            change_soundID.val("");
            change_soundID.attr("class", "soundOn");
            alert("Sound On");
        }
    }
    return div;
...
function setAudioElement(pn, file) {
    if (typeof(file) == 'undefined')
        file = pn;
    var html = "<source src='" + oStaticURL + "sound/" + file + ".mp3' />";
    html += "<source src='" + oStaticURL + "sound/" + file + ".ogg' />";
...
function startCountDown() {
    ...
    elTimebar.tmp = 700;
    elTimebar.style.width = "700px";
    timeID = setInterval("decTime()", 800);
}


function decTime() {
    elTimebar.tmp--;
    elTimebar.style.width = elTimebar.tmp + "px";
    if (elTimebar.tmp == 0) {
        if (el.checksound) {
            getAudioName("sound5").play();
        }
